...
const SVGS = [
  ...
  'stripe_wordmark.svg',
  'supabase.svg',
  'tensorflow-icon-dark.svg',
  'terraform.svg',
  'trpc.svg',
  'turborepo-icon-dark.svg',
  'ubuntu.svg',
  'upstash.svg',
  'vite.svg',
  'warp.svg',
...
const XConfig = {
  count: 150,
  colors: [0x000000, 0x808080, 0x8a2be2, 0x7a9a65],
  ...
  materialParams: {
    ...
    thickness: 2.0,
    transparent: true,
    color: 0x1a1a1a, // Dark tint to ensure text readability
  },
  minSize: 0.8,
  maxSize: 1.5,
  size0: 1.5,
...
class Z extends InstancedMesh {
  ...
  constructor(renderer: WebGLRenderer, params: Partial<typeof XConfig> = {}) {
    ...
    for (let i = 0; i < config.count; i++) {
      const svgFile = SVGS[i % SVGS.length]
      const map = textureLoader.load(`/svgs/${svgFile}`)
  ...
  update(deltaInfo: { delta: number }) {
    ...
    for (let idx = 0; idx < this.count; idx++) {
      ...
      if (idx === 0 && this.config.followCursor === false) {
        U.scale.setScalar(0)
        this.sprites[idx].scale.setScalar(0)
      } else {
        U.scale.setScalar(size)
        // Make the sprite fit nicely inside the sphere
        this.sprites[idx].scale.setScalar(size * 1.4)
      }
      U.updateMatrix()
      this.setMatrixAt(idx, U.matrix)
      if (idx === 0) this.light!.position.copy(U.position)

      this.sprites[idx].position.copy(U.position)
    }
...
export const AnimatedSvgBackground: React.FC<BallpitProps> = ({
  className = '',
  followCursor = false,
  ...props
}) => {
  const isMobile = useIsMobile()
  const count = props.count ?? (isMobile ? 50 : 150)
  ...
  return (
    <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none mask-[linear-gradient(to_bottom,transparent,black_25%,black_30%,transparent)]">
      {/* Dark overlay tint to guarantee text legibility */}
      <div className="absolute inset-0 z-10 pointer-events-none mix-blend-multiply dark:mix-blend-overlay bg-linear-to-r sm:bg-linear-to-l from-background via-background/60 to-transparent" />
      <canvas
        className={`${className} w-full h-full opacity-60 dark:-opacity-90`}
        ref={canvasRef}
      />
    </div>
  )
