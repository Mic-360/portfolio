...
function detectDocumentClass(preamble: string): 'bhaumic' | 'russell' {
    if (/\\documentclass\{bhaumic\}/.test(preamble) ||
...
function extractMetadataBhaumic(body: string): {
    ...
    if (!headerMatch) {
        return { firstName: '', lastName: '', position: '', phone: '', email: '', github: '', linkedin: '' }
    }
    ...
    return {
        ...
        position: (args[2] ?? '').replace(/\\textbar\{\}/g, '|').replace(/\\&/g, '&'),
        phone: args[3] ?? '',
        email: args[4] ?? '',
        github: args[5] ?? '',
        linkedin: args[6] ?? '',
...
function parseSkillsBlock(content: string): string {
    ...
    while ((match = regex.exec(content)) !== null) {
        const typeStart = content.lastIndexOf('{', match.index + match[0].length - 1)
...
function parseCertRowsBlock(content: string): string {
    ...
    while ((match = regex.exec(content)) !== null) {
        const nameStart = content.lastIndexOf('{', match.index + match[0].length - 1)
...
function parseEduEntries(content: string): string {
    ...
    while ((match = regex.exec(content)) !== null) {
        ...
        if (args.length >= 4) {
            ...
            const courseworkMatch = remaining.match(/\\textbf\{Relevant Coursework:\}([\s\S]*?)(?=\\|$)/)
            if (courseworkMatch) {
                const courses = processInline(courseworkMatch[1].replace(/\\textbullet\{\}/g, '·').trim())
