...
function extractJsonLdPinMeta(html: string): JsonLdPinMeta {
	...
	const title = cleanText(
		String(
			best.headline ?? best.title ?? best.name ?? '',
		),
	)

	const description = cleanText(
		String(
			best.description ?? best.articleBody ?? '',
		),
	)
...
async function fetchPinWithRetry(pinUrl: string): Promise<PinterestCreatedPin | null> {
...
export async function getPinterestCreatedPinsInternal(): Promise<PinterestCreatedPinsPayload> {
	...
	requestInFlight = (async () => {
		try {
			...
			const pins = pinResults.filter((pin): pin is PinterestCreatedPin => pin !== null)
